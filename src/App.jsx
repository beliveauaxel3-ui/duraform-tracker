...
export default function App() {
  ...
  const [transferGroup, setTransferGroup] = useState("all");
        ...
        <div className="overlay" onClick={() => setTransfer(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <label style={{ fontSize: 11, color: C.muted, fontWeight: 700, display: "block", textTransform: "uppercase", letterSpacing: "0.06em" }}>Quantités</label>
              {(() => {
                const selectedOutsideView = Object.entries(txForm.items).filter(([key, qty]) => {
                  if (!(qty > 0)) return false;
                  const ft = formTypes.find(f => f.key === key);
                  const inView = transferGroup === "all" || ft?.group === transferGroup;
                  return !inView;
                }).length;
                return selectedOutsideView > 0 ? (
                  <span style={{ fontSize: 11, color: C.accent, fontWeight: 600 }}>+{selectedOutsideView} sélectionné{selectedOutsideView > 1 ? "s" : ""} ailleurs</span>
                ) : null;
              })()}
            </div>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
              <button className={`group-tab ${transferGroup === "all" ? "active" : ""}`} style={{ padding: "6px 13px", fontSize: 12 }} onClick={() => setTransferGroup("all")}>Tous</button>
              {groups.map(g => (
                <button key={g.id} className={`group-tab ${transferGroup === g.id ? "active" : ""}`} style={{ padding: "6px 13px", fontSize: 12 }} onClick={() => setTransferGroup(g.id)}>{g.label}</button>
              ))}
              {formTypes.some(f => !f.group) && (
                <button className={`group-tab ${transferGroup === "none" ? "active" : ""}`} style={{ padding: "6px 13px", fontSize: 12 }} onClick={() => setTransferGroup("none")}>Sans groupe</button>
              )}
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
              {formTypes.filter(ft => transferGroup === "all" || (transferGroup === "none" ? !ft.group : ft.group === transferGroup)).map(ft => {
